...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setMessages(notificationsResponse.data.notifications)
      } catch (error) {
        console.error(error)
      }
    }
  ...
  const handleMarkAsRead = useCallback(async (messageId) => {
    try {
      const res = await axios.put(`/api/notifications/update/${messageId}`)
      if (res.status === 200) {
    ...
    <div className='w-[calc(100vw-333px)] max-w-full h-[96px] flex justify-between items-center bg-backgroundPrimary px-10 absolute left-0 top-0 z-[100] shadow-sm'>
      ...
      {pathname !== '/dashboard/rewards' && (
        <div className='flex justify-end items-center gap-6'>
          ...
          {/* Notification Popup */}
          {showNotifications && (
            <div className='absolute top-0 right-0 max-h-[100vh] bg-backgroundPrimary z-50 w-[40%] h-auto transition-all duration-200 ease-linear'>
              <div className='bg-backgroundPrimary flex justify-between items-center w-full h-[96px] px-10 border-b border-b-black border-opacity-50'>
